...
assert d["summary"]["approved"] == 0, "no skill may be approved without full DS-43 audit"
for s in d["skills"]:
    ...
    assert s["runtime_permissions"]["independent_execution_authority"] is False
print("skills: 0 approved, full DS-43 fields")
PY
ok "skills registry"
...
test -f docs/science/LUMEN_SCIENCE_FUSION_CANDIDATE_STATUS.md || fail "missing candidate status"
grep -q "NOT YET" docs/science/LUMEN_SCIENCE_FUSION_CANDIDATE_STATUS.md || fail "status must deny false 1.0"
ok "honesty docs"
