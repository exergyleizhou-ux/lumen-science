...
pub fn project_result_usage(result: &mut serde_json::Value, usage: &PromptUsage) {
    ...
    // Lumen cache stack: human-readable line + multi-provider profile note.
    // Never inject this into the system-prompt prefix — headless/status only.
    let cache_line = lumen_discipline::format_cache_line(lumen_discipline::CacheUsage {
        input_tokens,
        cache_read_tokens: cached_read_tokens,
        output_tokens,
    });
    let hit_ratio = lumen_discipline::hit_ratio(input_tokens, cached_read_tokens);
