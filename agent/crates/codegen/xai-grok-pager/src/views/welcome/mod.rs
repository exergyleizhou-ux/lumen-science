...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Lumen** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Lumen** VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    match &mode {
        ...
        VersionBadgeMode::HeroFooter => {
            // FINAL-5UX: never hard-code product channel "Beta"; show real channel label only.
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Lumen is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that Lumen
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Lumen may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
