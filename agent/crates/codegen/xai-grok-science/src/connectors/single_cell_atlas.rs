//! Single Cell Expression Atlas (EBI). Seam: S3.
...
pub fn search_path(query: &str) -> String {
    format!(
        "/gxa/sc/json/experiments?speciesQuery={}&keyword={}",
...
pub fn parse_search(bytes: &[u8]) -> crate::Result<ParsedResponse> {
    ...
    let v: serde_json::Value = serde_json::from_slice(bytes).map_err(|e| {
        ScienceError::Invalid(format!("single-cell-atlas: malformed JSON: {e}"))
    })?;
    let experiments = v
        .get("experiments")
    ...
    for ex in &experiments {
        ...
        recs.push(RetrievedRecord {
            ...
            container: "Single Cell Expression Atlas".to_owned(),
            url: format!("https://www.ebi.ac.uk/gxa/sc/experiments/{id}"),
        });
    }
    Ok(ParsedResponse {
...
impl ProtocolAdapter for SingleCellAtlasAdapter {
    ...
    fn parse_responses(&self, e: &[FetchExchange]) -> crate::Result<ParsedResponse> {
        if e.len() != 1 {
            return Err(ScienceError::Invalid(format!(
                "single-cell-atlas: expected 1 exchange, got {}",
                e.len()
            )));
        }
...
mod tests {
    ...
    const OK: &[u8] = br#"{"experiments":[{"experimentAccession":"E-HCAD-1","experimentDescription":"sc demo"}]}"#;
    ...
    fn wrong_exchange_count() {
        assert!(SingleCellAtlasAdapter.parse_responses(&[]).is_err());
    }
}
